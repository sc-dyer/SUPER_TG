...
!
!
!This program depends on hprogs.f90, help.f90, and platf-unix/osx/win.f90 
!A lot of subroutines and functions are included in these files
!Might depend on activi.f90, dasave.f90, dbread.f90, fsol.f90, gcalc.f90, gmini.f90,prinin.f90, and prtcal.f90 
!Not sure, havent looked into these ones yet but they are included during compilation

!This is an edited version of THERIA_G to be used as a module. THERIA_G itself is a callable subroutine which can take in a P-T-t-path in the form of three seperate arrays
!of equivelant length as well as the file name (and directory if applicable) of the THERIN, thermodynamic database, CSD, and DIF files
!If you wish to modify this file (good luck!) I will note the line numbers where I made my changes here
!P-T-t-path input @ line 998
!THERIN file input @ line 155 also dat unit in theriak.ini
!Database file input @ line 129
!CSD file input @ line 955
!DIF file input @ line 931

!I tried making this as a module but it seems the tools used are too archaic, this may cause problems in super_tg
...
MODULE theriag_mod
      ...
      SUBROUTINE run_theriag(p_listIn, tem_listIn, time_listIn, therin_file, database_file, csd_fileIn, dif_fileIn) 
         !USE , INTRINSIC :: ISO_FORTRAN_ENV
         !IMPLICIT NONE
         !INCLUDE 'theriak.cmn'
         !INCLUDE 'files.cmn'

         CHARACTER(LEN=*) :: therin_file, database_file, csd_fileIn,dif_fileIn
         REAL(KIND=8), DIMENSION(:) :: p_listIn,tem_listIn,time_listIn
         !Maximum number if layers and garnet generations
   ...
   !-----END OF COMMON VARIABLES
         ...
         REAL*8 FF
         CHARACTER*16 CH16
         CHARACTER*500 CH001,CH002,SYREC,CHIN(2),ZEITSTRING !ZEITSTRING = time string
         ...
         ALLOCATE(csd_file,SOURCE=csd_fileIn)
         ALLOCATE(dif_file,SOURCE=dif_fileIn)
         ALLOCATE(p_list,SOURCE=p_listIn)
         ALLOCATE(tem_list,SOURCE=tem_listIn)
         ALLOCATE(time_list,SOURCE=time_listIn)
   ...
   !       CALL LABLA(DBNAME,I001)
         ...
         DBNAME = ADJUSTL(TRIM(database_file)) !Hopefully this is where the database name is defined
   ...
   !------------------
         ...
         ! akzess=' '
         ! state=' '
         ! call openfile(j,ierr) !Latex stuff $a^b = \int{\phi^2}$ 
         ! if(ierr.ne.0) STOP
   !------------------
   !     open UNIT=dbs
   !------------------
...
!********************************
      SUBROUTINE GROWG
         ...
         PARAMETER (MAXLAY=1000,MAXGEN=100)
   ...
   !    AVMn,AVFe,AVMg               activation volume for diffusion in cm^3/mol
         OPEN (UNIT=44,FILE=dif_file,STATUS='OLD')
   ...
   !----
    ...
    1014 FORMAT ( &
         ...
         !Added to theriag_mod by Sabastien
         !uses Subroutine inputs instead of reading theriag_PTt.txt

         IF(SIZE(p_list) == SIZE(tem_list) .AND. SIZE(tem_list) == SIZE(time_list)) THEN
            DO i=1, SIZE(p_list)
               TSEG(i) = tem_list(i)
               PSEG(i) = p_list(i)
      ...
      !Commented out as a part of theriag_mod 
     ...
     !        NSEG=NSEG+1
     !        !Change FF to whatever input is given for P T t, remove GELI
     !        TSEG(NSEG)=FF !Temperature segment
     !        CALL GELI(SYREC,FF)
     !        PSEG(NSEG)=FF !Pressure segment
     !        CALL GELI(SYREC,FF)
     !        TIMSEG(NSEG)=FF !Time segment
...
!-----search for layer growth
      ...
      VOB=VAIM
      VUN=0.0D0
   10 IF (DABS((VOB-VUN)/VAIM).LT.1D-12.OR.DABS(TOB-TUN).LT.1D-12) &
       GOTO 11
      TC=(TUN+TOB)/2.0D0
      P=(PUN+POB)/2.0D0
      CALL NURVONPT
...
!----- end of GLOBAL DATA Common variables
      ...
      CHARACTER*10000 BUNA,BUNA2,BUNA3,BUNA4,ZTHERIN
!-----
...
!-----
!********************************
      SUBROUTINE DIFFUS(TNOW,PNOW,DIFTIM)
      IMPLICIT NONE
!-----
      INTEGER*4 MAXLAY,MAXGEN
      PARAMETER (MAXLAY=1000,MAXGEN=100)
...
!----- end of GLOBAL DATA Common variables
      ...
      INTEGER*4 I001,TSLOOP,ITSLOOP,OPSY(3),TRY
