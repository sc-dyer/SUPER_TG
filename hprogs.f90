...
!******************************
      SUBROUTINE LABLA(CH,II)
         !Finds the last occurence of a character that isn't ' ' in CHARACTER CH
         !Returns index II, returns 0 is there isnt any
         IMPLICIT NONE
         CHARACTER*(*) CH
         INTEGER*4 II,I,LAE
         LAE=LEN(CH)
...
!******************************
      SUBROUTINE GELI(CH,FF)
         !Subroutine that gets the next number in CH and outputs it as FF
         !Also removes the number it takes from CH to parse out the whole file
         IMPLICIT NONE
         include 'files.cmn'
         CHARACTER*(*) CH
         CHARACTER*250 CH001
         ...
         CALL FIBLA(CH,I001)
         IF (I001.EQ.0) THEN !Check if end of file
            FF=0.0D0
            RETURN
         END IF
         CH001=CH(I001:)!Takes all of CH after any leading spaces
         I001=INDEX(CH001,' ') !Get index of first space
         CH002=CH001(1:I001) !First number 
         I002=INDEX(CH002,'/') !Check if its a divided number?
         IF (I002.EQ.0) THEN
            CH016=CH001(1:I001-1)
            READ (UNIT=CH016,FMT='(BN,D16.0)',ERR=999) FF !Set FF to the first number as a double?
         ELSE
            ...
            FF=F1/F2
         END IF
         CH=CH001(I001:) !Set CH to the rest of the file after the first number
         RETURN
